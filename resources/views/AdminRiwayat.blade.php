...
@section('content')
    ...
    <body class="font-poppins">
        <div class="text-left ml-[21rem] mt-4 mb-4">
            <ul>
                <li class="mb-8 text-xl font-bold">Riwayat</li>
            </ul>
        </div>
        <table class="table-fixed border border-slate-200 ml-[21rem] font-medium">
            ...
            @foreach ($riwayats as $riwayat)
                <tbody class="text-center">
                    <tr>
                        <td class="p-2 text-xs border border-slate-200">{{ ($riwayats->currentPage() - 1) * $riwayats->perPage() + $loop->iteration }}</td>
        ...
        <div class="w-6/12 bg-white modal-box">
            <div class="bg-white">
                ...
                <div class="flex justify-end mt-4">
                    <button type="button" class="mr-4 text-white bg-gray-500 border btn" onclick="document.getElementById('modaldetail{{$riwayat->Id_Riwayat}}').close();">Batal</button>
                </div>
            </div>
        </div>
    </dialog>
