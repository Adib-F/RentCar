        ...
        <table class="table-fixed border border-slate-200 ml-[21rem] font-medium">
            <thead class="">
                <tr>
                    ...
                    <th class="p-2 text-xs border border-slate-200">Nama Lengkap</th>
                    <th class="p-2 text-xs border border-slate-200">Username</th>
                    <th class="p-2 text-xs border border-slate-200">No Handphone</th>
                    <th class="p-2 text-xs border border-slate-200">NIK</th>
                    <th class="p-2 text-xs border border-slate-200">Roles</th>
                    <th class="p-2 text-xs border border-slate-200">Email</th>
                    <th class="p-2 text-xs border border-slate-200">Aksi</th>
                </tr>
            </thead>

            <!-- Menampilakan data pengguna dari database -->
            @foreach ($penggunas as $nomor => $pengguna)
            ...
            <div class="w-6/12 bg-white modal-box">
                <form method="POST" action="{{ route('addPengguna') }}">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div><label for="nama"
                                class="text-sm font-bold leading-tight tracking-normal text-gray-800">Nama Lengkap</label><br>
                                @error('nama_lengkap')
                    ...
                    <div class="w-6/12 bg-white modal-box">
                        <form method="POST" action="{{ route('updatePengguna', $pengguna->Id_Pengguna) }}">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label for="roles" class="text-sm font-bold leading-tight tracking-normal text-gray-800">Roles</label><br>
                                    @error('roles')
                                        <span class="text-xs text-red-500">{{ $message }}</span>
                                    @enderror
                                    <select id="roles" name="roles"
                                        class="flex items-center w-full h-8 pl-3 mt-1 mb-2 text-sm text-gray-600 bg-white border border-gray-300 rounded font-norma focus:outline-none focus:border focus:border-black">
                                        <option value="{{ $pengguna->Roles }}" selected disabled hidden>Pilih Roles</option>
                                        <option value="Admin" {{ old('roles', $pengguna->Roles) == 'Admin' ? 'selected' : '' }}>Admin</option>
                                        <option value="Pengguna" {{ old('roles', $pengguna->Roles) == 'Pengguna' ? 'selected' : '' }}>Pengguna</option>
                                    </select>
                                </div>
